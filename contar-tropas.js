...
try{
	...
	function fnTranslate(id){
		var translation={
			"en":[
				"Total Ofensivos+Aristo",
				"Total Defensivos+Aristo",
				"Otros Aristos",
				"Ataque Completo",
				"Ataque en 3/4",
				"Ataque en 1/2",
				"Ataque en 1/4",
				"Ataque Full - Catapulta",
				"Defensa Completa",
				"Defensa en 3/4",
				"Defensa en 1/2",
				"Defensa en 1/4",
	...
	function fnLogVersionInfo(){
		fnDebugLog("=========================");
		fnDebugLog("Sumario de Tropas : " + strVersion);
		fnDebugLog("=========================");
		fnDebugLog("Premium: "+(win.game_data.isPremium?"Si":"No"));
		fnDebugLog("Iglesia : "+(fnHasChurch()?"Si":"No"));
		fnDebugLog("Estatua : "+(fnHasPaladin()?"Si":"No"));
		fnDebugLog("Arqueros : "+(fnHasArchers()?"Si":"No"));
		fnDebugLog("Milicia: "+(fnHasMilitia()?"Si":"No"));
		fnDebugLog("Sitteur : "+(win.location.href.match(/t\=\d+/i)?"Si":"No"));
		fnDebugLog("=========================");
		...
		fnDebugLog("Navegador : "+navigator.userAgent);
		fnDebugLog("=========================");
...
*/	
		...
		docSource += "<html>\n";
		docSource += "\t<head>\n";
		docSource += "\t\t<title>Sumário de Tropas</title>\n";
...
*/
		...
		docSource += "\t\t\t\t\t\t\t<h2>" + fnTranslate(curGroup++) + "<sup><span style=\"font-size:small;\">" + strVersion + "</span></sup><sub><span style=\"font-weight:100;font-style:italic;text-decoration:none;font-size:x-small;\"><a href=\"http://www.hemispheria.net\" target=\"_blank\"> Cr\351dit : Chawento</a></span></sub></h2>\n";
		docSource += "\t\t\t\t\t\t\t<hr>\n";
		...
		docSource += "\t\t\t\t\t\t\t\t\t<tr class=\"" + "row_b" + "\"><td><span>Poblacion:</span></td><td style=\"white-space:nowrap;\"><span> " + summary.unitTotal.population + "</span></td></tr>\n";
		docSource += "\t\t\t\t\t\t\t\t</table>\n";
		...
		var popup=win.open('about:blank','Chawento','width=460,height=730,scrollbars=yes');
	...
	if(win.game_data.mode != "units"){
		throw("Ejecutar En Vision General »» Tropas »» Todas\n [ Opcion: Todos | Próprios | En Pueblo | Fuera | En Transito | Defensa | Apoyos ]");
	}
