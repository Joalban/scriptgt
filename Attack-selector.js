...
table.tHead.innerHTML = (x.replace("</tr>", "") + "<th>Pueblo</th><th>Marcar Todo<input type='checkbox' id='selectAllBox' onClick='javascript: selectAll(); void(0)'></input></th><th>Invertir<input type='checkbox' id='invertListBox' onClick='javascript: invertAll(); void(0)'></input></th>");
...
mainTable.innerHTML = "<br><table><tr><th>Lista Formato Experimental</th><td><input type='text' name='experimentalListFormat' value='"+defaultExString+"' id='experimentalFormatBox' /><br>Ins\351rer<select id='experimentalSelectBox' onChange=\"javascript: document.getElementById('experimentalFormatBox').value += document.getElementById('experimentalSelectBox').value; void(0);\"><option value=''>Seleccionar</option><option value='{COUNTER} '>Contador</option><option value='{COORDS} '>Coordonadas</option><option value='[village]{COORDS}[/village] '>BBCode Coordenadas</option><option value='{VILLAGE LINK} '>Pueblo</option><option value='{TARGET PLAYER} '>Nombre Jugador</option><option value='[player]{TARGET PLAYER}[/player]'>BBCode Jugador</option><option value='{POINTS} '>Puntos</option><option value='{DV}'> - Dividir -</option><option value='{NL}'>Nueva Linea</option></select></td></tr><tr><th></th><td><input type='button' value='OK' onClick='javascript: outputExperimentalList(); void(0);' /></td></tr></table>" + mainTable.innerHTML;
...
/*Text Match Table*/
mainTable.innerHTML = "<br><table><tr><th>Busqueda</th><td><input type='text' id='textMatchBox' size='17' value='Texto a Buscar'/></td></tr><tr><th></th><td><input type='button' value='Buscar' onClick='javascript: byText(); void(0);'></input></td></tr></table><br>" + mainTable.innerHTML;
/*End Text Match Table*/

/*Points Table*/
mainTable.innerHTML = "<br><table><tr><th>Puntos Minimos</th><td><input type='text' id='minPointsBox' size='8' value='0'/></td></tr><tr><th>Puntos Maximos</th><td><input type='text' id='maxPointsBox' size='8' value='13000'/></td></tr><tr><th></th><td><input type='button' value='OK' onClick='javascript: byPoints(); void(0);'></input></td></tr></table><br>" + mainTable.innerHTML;
/*End Points Table*/

/*Travel Time Table*/
mainTable.innerHTML = "<br><table><tr><th>Timepo Trayecto</th><td>HMS :<input type='text' size='3' value='24' id='maxH'></input>:<input type='text' size='3' value='00' id='maxM'></input>:<input type='text' size='3' value='00' id='maxS'></input></td></tr><tr><th>Coordonadas</th><td>X:<input type='text' size='5' value='"+game_data.village.coord.split('|')[0]+"' id='centralX1'></input> Y:<input type='text' size='5' value='"+game_data.village.coord.split('|')[1]+"' id='centralY1'></td></tr><tr><th><select id='troopTypeBox'><option value='NONE'>Unit\351</option><option value='spear'>Lanceros / Hachas / Arqueros</option><option value='sword'>Espadas</option><option value='spy'>Espias</option><option value='light'>Ligeros</option><option value='marcher'>Arqueros Montados</option><option value='heavy'>Pesados</option><option value='ram'>Ariete/Catapulta</option><option value='snob'>Noble</option></select></th><td><input type='button' value='GO!' onClick='javascript: byTravelTime(); void(0);'></input></td></tr></table><br>" + mainTable.innerHTML;
/*End Travel Time Table*/

/*Proximity Table*/
mainTable.innerHTML = "<br><table><tr><th>Coordenadas</th><td>X:<input type='text' size='5' value='"+game_data.village.coord.split('|')[0]+"' id='centralX'></input> Y:<input type='text' size='5' value='"+game_data.village.coord.split('|')[1]+"' id='centralY'></td></tr><tr><th></th><td>Cases:<input type='text' size='5' value='25' id='maxFields'></input><input type='button' value='GO!' onClick='javascript: byFields(); void(0);'></input></td></tr></table><br>" + mainTable.innerHTML;
/*End Proximity Table*/


/*Zone Table*/
mainTable.innerHTML = 
  "<br><br><table><tr><th>Zone Objetivo (Baja)</th><td>X:<input type='text' size='5' value='000' id='lowestX'></input> Y:<input type='text' size='5' value='000' id='lowestY'></input></td></tr><tr><th>Zona Objetivo (Alta)</th><td>X:<input type='text' size='5' value='999' id='highestX'></input> Y:<input type='text' size='5' value='999' id='highestY'></input></td></tr><tr><th>Por Continente</th><td>C<input type='text' size='4' value='55' id='inputK'></input><input type='button' value='Continent' onClick='javascript: selectByK(); void(0);'></input></td></tr><tr><th></th><td><input type='button' value='Zone' onClick='javascript: selectZone(); void(0);'></input></td></tr></table><br>" + mainTable.innerHTML;
/*End Zone Table*/

/*Selection Mode Table*/
mainTable.innerHTML = "<br><br><table><tr><th>Modo Seleccion</th><td>Ajouter:<input type='radio' name='selectionFormat' value='Add' id='addRadBut' /><br>Supprimir:<input type='radio' name='selectionFormat' value='Remove' id='removeRadBut' /><br>Remplazar:<input type='radio' name='selectionFormat' value='Replace' id='replaceRadBut'  /></td></tr></table><br>" + mainTable.innerHTML;
