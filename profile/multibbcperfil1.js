; (() => {
    try {
        ...
        if (window.location.href.indexOf('screen=info_player') < 0) {
            alert("Ejecutar desde Perfil Jugador!");
        }
        ...
        console.log('Este Pueblo es: ', localCoord);
        ...
        coords.forEach((c) => {
            console.log('Distancia de ' + JSON.stringify(localCoord) + ' a ' + JSON.stringify(c) + ' es ' + dist(c, localCoord));
        });
        ...
        function makeBbcodeTable(coords, $table) {
            ...
            header = `
                [**]
				${options.useNaming ? 'Jugador[||]' : ''}
				${options.useClaimsTag ? 'Pueblo[||]' : ''}
