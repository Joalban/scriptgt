...
var addText = function (texte) {
	text_area.appendChild(document.createTextNode(texte));
		...
		/*______*/
	for (var j = 1, j2 = 2; j < tag_td.length, j2 < tag_td.length; j = j + 3, j2 = j2 + 3)	{
		name_villages.push("[claim]" + tag_td[j].textContent + "[/claim]");
		points.push(tag_td[j2].textContent);
	}
	for (var i = 0, k = 0; i < tag_td.length / 3, k < name_villages.length; i++, k++)	{
			format[0] += "\n" + d + i + s + name_villages[k] + s + points[k] + s + "";
			format[1] += "\n" + d + i + s + name_villages[k] + s + points[k];
			format[2] += "\n" + d + i + s + name_villages[k] + s + "";
